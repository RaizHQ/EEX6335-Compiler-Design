...
ASTNode* createID(char *name) {
    ASTNode *node = createNode(NODE_INDICE);
...
void print_branch(FILE *fp, ASTNode *node, char *prefix, int isLast) {
    ...
    switch (node->kind) {
        ...
        case NODE_CLASS_DECL:   fprintf(fp, "CLASS_DECL [Name: %s]\n", node->name); break;
        case NODE_IMPL_DEF:     fprintf(fp, "IMPLEMENT [Name: %s]\n", node->name); break;
        case NODE_FUNC_DEF:    fprintf(fp, "FUNC_DEF [Name: %s] [Return: %s]\n", 
                               node->name, 
                               node->type_name ? node->type_name : "void"); break;
        case NODE_FUNC_HEAD:    fprintf(fp, "FUNC_HEAD [Name: %s]\n", node->name); break;
        case NODE_BLOCK:        fprintf(fp, "BLOCK\n"); break;
        case NODE_VAR_DECL:     fprintf(fp, "VAR_DECL [Name: %s, Type: %s]\n", node->name, node->type_name); break;
        ...
        case NODE_EXPR_BINOP:   fprintf(fp, "EXPR_BINOP (%s)\n", node->op); break;
        case NODE_EXPR_UNOP:    fprintf(fp, "EXPR_UNOP (%s)\n", node->op); break;
        case NODE_FUNC_CALL:    fprintf(fp, "CALL [Func: %s]\n", node->name); break;
        case NODE_INDICE:   fprintf(fp, "ID [%s]\n", node->name); break;
